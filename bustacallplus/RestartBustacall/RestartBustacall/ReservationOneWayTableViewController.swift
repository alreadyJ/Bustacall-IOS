//
//  ReservationOneWayTableViewController.swift
//  RestartBustacall
//
...
extension Date {
    func string2(format: String) -> String {
...
class ReservationOneWayTableViewController: UITableViewController {
    ...
    @IBAction func datePickerBtn(_ sender: Any) {
        ...
        DatePickerDialog().show("출발날짜 설정", doneButtonTitle: "확인", cancelButtonTitle: "취소", minimumDate: currentDate, maximumDate: limit, datePickerMode: .date) { (date) in
            if let dt = date {
                let str = dt.string2(format: "yyyy. MM. dd. EEE")
...
//    }
    ...
    */
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "startOneWaySegue"{
        ...
        if segue.identifier == "endOneWaySegue"{
